  ...
  useEffect(
    () => {
      if (volumes) {
        ...
        setData(
          volumes.map(d => {
            const { timestamp, volume, volume_by_chains } = { ...d }
            return {
              ...d,
              id: timestamp,
              ...
              value: volume,
              value_string: numberFormat(volume, '0,0.00a'),
              values: volume_by_chains,
              ...Object.fromEntries(
                toArray(volume_by_chains).map(d => {
                  const { id, volume } = { ...d }
                  return [id, volume]
                })
              ),
            ...
            >
              <defs>
                <linearGradient id={gradient_id} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="25%" stopColor="#ef4444" stopOpacity={0.95} />
