...
export const daily = async (sdk, params) => {
  ...
  while (hasMore) {
    ...
    let response = await graphql({ ...params, api_type: 'analytic', query })

    if (response?.data?.error && sdk) {
      try {
        response = {
          data: {
            ...await sdk.querySubgraph(params?.chain_id, query),
          },
        }
      } catch (error) {}
    }
